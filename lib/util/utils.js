...
module.exports = {
 ...
 /*
  * createTagDir - function to create a directory specific for each tag
  * within /output/data dir. Cache the dir name after creation.
  * 
  * @param - string
  * @return boolean
  * 
  */
  createTagDir: function (tagname){

	if(!tagname){
	  return false;
	}

	tagname = 'TAG_' + tagname.replace(/[^a-zA-Z 0-9]+/g,'');
	
	// Check if the dir name is in the cache
	if(fileCache[tagname] !== 'undefined'){
		return true;
	} 
	
	fs.mkdir('../output/data/' + tagname, 0777, function (err) {
	  if (err) {
	    console.log(err);
	    process.exit();
	  } else {
	  	fileCache[tagname] = tagname;
	    return true;
	  }
	});
  ...
  cleanStopWords: function (string) {
  	...
  	var filtered  = string.toLowerCase().split( /\b/ ).filter( function( v ){
		...
		filtered.forEach(function(item) {
			// Drop empty strings and strings that are just numbers e.g. '76876589'
			if(item.indexOf(' ') === -1 && !/^\d+$/.test(item))
			  out.push(item);
		});
