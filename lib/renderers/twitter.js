...
module.exports = {
	...
	renderInfluencerModel : function() {
		...
		if(utils.isEmpty(influencers.samplemodel) === false){
			...
			dump_influencer_example += 'username,full_name,description,retweets,followers,mentions,replies,logarithm' + "\n";
			...
			for(var userIndx in influencers.samplemodel){
				if(influencers.samplemodel[userIndx] !== undefined && typeof(influencers.samplemodel[userIndx]) === 'object'){	
					...
					var total = influencers.samplemodel[userIndx]['retweetCount'] + influencers.samplemodel[userIndx]['followersCount'] +
								influencers.samplemodel[userIndx]['mentionsCount'] + influencers.samplemodel[userIndx]['repliesCount'];
					
					var log = Math.log(total) / Math.LN10;

					dump_influencer_example += userIndx + ','; // twitter handle
					dump_influencer_example += '"' + influencers.samplemodel[userIndx]['name'] + '",'; // full name
					dump_influencer_example += '"' + influencers.samplemodel[userIndx]['description'] + '",'; // desc
					dump_influencer_example += influencers.samplemodel[userIndx]['retweetCount'] + ',';
					...
					dump_influencer_example += log;
					dump_influencer_example += "\n";	
				
				}
			}
			
			filehandler.save('twitter_influencers_example_model.csv', dump_influencer_example);
	...
	renderRetweetRetweeted : function() {
		...
		if(utils.isEmpty(influencers) === false){
			...
			filehandler.save('twitter_influencers_retweeted.csv', dump_retweet_influencer);
