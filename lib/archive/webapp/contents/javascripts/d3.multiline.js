...
d3.csv("data/tag_volume_by_day.csv", function(error, data) {
 ...
 data.sort(function (a, b) {
  ...
  var tags = color.domain().map(function(name) {
    return {
      name: name,
  ...
  y.domain([
    d3.min(tags, function(c) { return d3.min(c.values, function(v) { return v.volume; }); }),
    d3.max(tags, function(c) { return d3.max(c.values, function(v) { return v.volume; }); })
  ]);
