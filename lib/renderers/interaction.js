...
module.exports = {
  ...
  // Render type summary volumes and percentages
	renderTypes : function() {
		
		if(utils.isEmpty(interaction.types) === false){
			var dump_all_types = '';
		  dump_all_types += 'type,volume,percentage_of_total' + "\n";
		  	
		  // Get the total number of gener profiles collected
		  var type_total = 0;
		  for(var typeTotIndex in interaction.types){
		  		type_total += interaction.types[typeTotIndex];
		  }
			
		  for(var typeIndex in interaction.types){
		  	dump_all_types +=  typeIndex + ',' + interaction.types[typeIndex] + ',' + (interaction.types[typeIndex]/type_total*100) + "\n";
		 	}
			filehandler.save('type_summary.csv', dump_all_types);
		}
