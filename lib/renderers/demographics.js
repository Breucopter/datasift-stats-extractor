...
module.exports = {
	...
	renderDemographics : function(data) {
		...
		if(utils.isEmpty(demographics.range) === false){
		  var dump_age_range = '';
		  dump_age_range += 'age_range,volume,percentage_of_total' + "\n";
		  
		  var total_age_profiles = 0;
			for(var allRangeIndex in demographics.range){
		  	total_age_profiles += demographics.gender[allRangeIndex];
		 	}
		  
		  
		  for(var rangeIndex in demographics.range){
		  	dump_age_range +=  rangeIndex + ',' + demographics.range[rangeIndex] + ',' + (demographics.range[rangeIndex]/total_age_profiles*100) + "\n";
		 	}
