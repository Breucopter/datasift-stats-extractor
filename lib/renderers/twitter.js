...
module.exports = {
	...
	renderInfluencerModel : function() {
		...
		dump_influencer_example += 'name,retweetCount,followers,mentions,replies' + "\n";
		...
		for(var userIndx in influencers.samplemodel){
			if(influencers.samplemodel[userIndx] !== undefined && typeof(influencers.samplemodel[userIndx]) === 'object'){	
				dump_influencer_example += userIndx + ',';

				for(var userdata in influencers.samplemodel[userIndx]){
					dump_influencer_example += influencers.samplemodel[userIndx][userdata] + ',';
				}
