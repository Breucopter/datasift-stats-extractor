...
module.exports = {
  ...
  save: function (name, content) {
    var fs = require('fs');
    fs.writeFile("output/" + name, content, function(err) {
      if(err)
		console.log(err);
	}); 
  },
  ...
  cleanFiles: function() {
      try { var files = fs.readdirSync('output/'); }
      catch(e) { return; }
      if (files.length > 0)
        for (var i = 0; i < files.length; i++) {
          var filePath = 'output/' + files[i];
          if (fs.statSync(filePath).isFile() && filePath.indexOf('gitignore') === -1)
